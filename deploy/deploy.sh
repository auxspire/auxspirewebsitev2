#!/usr/bin/env bash
# Deploy Auxspire website to VPS. Run from repo root. Requires SSH key (vps or root@72.61.227.53).
set -e
cd "$(dirname "$0")/.."
...
scp deploy/nginx.conf "$VPS:/etc/nginx/sites-available/auxspire-app.conf" 2>/dev/null || true
...
rsync -avz --exclude node_modules --exclude .git . "$VPS:/var/www/auxspire/" 2>/dev/null || \
  { tar cf - . | ssh "$VPS" "cd /var/www/auxspire && tar xf -"; }
